...
class WireCellToolkit(Package, CudaPackage):
    ...
    version("0.23.0", sha256="53712dd4bea79900fc86eee44779cdd9644be29354569f2b4368a7841a62b57")
    ...
    variant('nvtx',default=False,
            description="Add support for NVTX")
    #Conflict for nvtx
    conflicts('+nvtx', when='~cuda',msg='NVTX support requires CUDA')
    ...
    def patch(self):
        # CUDA 12 / NVTX3 does not provide legacy libnvToolsExt.so.
        # Remove nvToolsExt from libtorch library detection.
        filter_file(
                "            libs += ['nvToolsExt']",
        ...
        # header file  TorchnvToolsExt are in nvtx3 directory in newer version of cuda
        for header in [
                "spng/inc/WireCellSpng/TorchnvTools.h",
                "pgraph/inc/WireCellPgraph/TorchnvTools.h",]:
            filter_file(
    ...
    def install(self, spec, prefix):
        ...
        if spec.satisfies('+torch'):
            ...
            torch_include = "{0}/include,{0}/include/torch/csrc/api/include".format(torchbase)
            torch_lib = "{0}/lib".format(torchbase)

            if spec.satisfies('+cuda'):
                cuda_target = join_path(
                    spec["cuda"].prefix,
                    "targets",
                    f"{spec.target.family}-{spec.architecture.platform}",
                )

                torch_include += "," + join_path(cuda_target, "include")
                torch_lib += "," + join_path(cuda_target, "lib")

            cfg.append("--with-libtorch={0}".format(torchbase))
            cfg.append("--with-libtorch-include={0}".format(torch_include))
            cfg.append("--with-libtorch-lib={0}".format(torch_lib))
            #cfg.append( "--with-libtorch={0}".format(torchbase) )
